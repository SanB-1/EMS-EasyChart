...
treatment and transport.'''
        ...
        tmethod2 = self.tmethod2_list[int(tmethod2)]
        return(f"""****************************************************************************
...
Ambulance {str(unit)} dispatched by {str(dispatcher)} for a BLS transport to take the patient from {origin} to\
{destination}.

ATF: {str(age)} Year Old {sex} {pos}; presents with a self-maintained, patent airway and is breathing at a normal rate \
and rhythm, with equal, bilateral chest rise. Pt appears to be perfusing adequately.
    
HPI: {hpi}.
    
BLS assessment:  See assessment TAB
    
Transfer:  Pt is transferred to our stretcher via {tmethod1}.  Pt is assisted to a position of comfort and secured in \
with all safety straps. The side rails are placed up and locked. Pt is taken to the ambulance and loaded in while still\
 on stretcher, stretcher secured via locking floor mount
    
Transport: During transport pt is monitored for changes in vitals or alertness, or any new complaints. Pt rests during \
transport. Transport occurs without incident.
    
Destination: When the crew arrives at the destination, pt is removed from the ambulance while still on stretcher and \
taken into {where}. Pt {tmethod2} onto bed. Upon transfer of care, patient offers no new complaints and no further \
changes noted in patient's condition beyond what has been documented elsewhere.
